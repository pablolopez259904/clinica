...
$db_conexionREdit->real_query("SELECT id_puesto as id, puesto FROM puestos WHERE id_puesto = $idEdit;");
$resultadoREdit = $db_conexionREdit->use_result();
$filaPuestosEdit = $resultadoREdit->fetch_assoc();
...
<body background="../../imgs/fondoUsuarios.jpg" style="background-size: cover; background-repeat: no-repeat; margin: 0px; height: 100%;">
    <div class="col-md-7" style="margin-left: 50px;">
        <div style="padding:10px; background-color: #1B1F78; color:white; margin-top: 2em;">
            <h3 class="text-center">Editar puesto</h3>
        </div>
        <div style="padding:10px; background-color: white; width: 100%;">
            <form action="" method="POST">
                <div class="row">
                    <input type="hidden" name="id" id="id" value="<?php echo $filaPuestosEdit['id']; ?>">
                    <div class="col-md-12">
                        <label for="lbl_puesto" class="form-label"><b>*Puesto</b></label>
                        <input type="text" name="txt_puesto" id="txt_puesto" class="form-control" value="<?php echo $filaPuestosEdit['puesto']; ?>">
                    </div>
                </div>
                <div style="margin-top: 1em;">
                    <a href="puestos.php" class="btn btn-success">Regresar</a> &nbsp;&nbsp;
    ...
    if (isset($_POST["btn_editar"])) {
        include '../../controllers/empleados/editarPuesto.php';
    }
    ?>
